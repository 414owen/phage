...
main :: IO ()
main = let withPrelude = readFile prelFile >>= run False core prelFile in
  getArgs >>= \args -> case args of
    (b : xs) -> readFile b >>= \program ->
      withPrelude >>= \baseSymTab ->
        run False baseSymTab b program >> pure ()
    _ -> withPrelude >>= repl
